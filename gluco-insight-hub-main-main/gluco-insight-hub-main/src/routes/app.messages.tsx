      ...
      <Card className="grid md:grid-cols-3 h-[600px] overflow-hidden">
        <div className="border-r border-border divide-y divide-border overflow-y-auto">
          {threads.map((t, i) => (
            <button
              key={i}
              onClick={() => setActive(i)}
              className={`w-full text-left p-4 hover:bg-secondary/40 ${active === i ? "bg-secondary/60" : ""}`}
            >
        ...
        <div className="md:col-span-2 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            <div className="flex">
              <div className="bg-secondary px-3 py-2 rounded-2xl text-sm max-w-[70%]">
                Hello! How are you feeling today?
              </div>
            </div>
            <div className="flex justify-end">
              <div className="bg-primary text-primary-foreground px-3 py-2 rounded-2xl text-sm max-w-[70%]">
                Doing well, glucose has been stable.
              </div>
            </div>
            <div className="flex">
              <div className="bg-secondary px-3 py-2 rounded-2xl text-sm max-w-[70%]">
