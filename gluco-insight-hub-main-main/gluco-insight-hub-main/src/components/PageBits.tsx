...
export function ListCard({ items }: { items: { title: string; sub?: string; right?: ReactNode; icon?: any; tint?: string }[] }) {
  return (
    <Card className="divide-y divide-border">
      {items.map((it, i) => (
        <div key={i} className="p-4 flex items-center gap-4 hover:bg-secondary/30 transition">
          {it.icon && <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${it.tint ?? "bg-primary/15 text-primary"}`}><it.icon className="w-5 h-5" /></div>}
